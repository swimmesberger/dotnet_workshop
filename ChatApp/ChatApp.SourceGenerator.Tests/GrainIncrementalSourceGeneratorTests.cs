...
public sealed class ChatRoomGrain : IGrain {
    ...
    public Task TriggerChatRoomAsync(int chatRoomId) {
        return Task.CompletedTask;
    }
}";
...
internal sealed class TriggerChatRoomCommand : IRequest {
    public required int ChatRoomId { get; init; }
}
...
public sealed class ChatRoomClient {
    ...
    public async ValueTask TriggerChatRoomAsync(int chatRoomId) {
        await _actor.Ask(new TriggerChatRoomCommand() {
            ChatRoomId = chatRoomId
        });
    }
}
        ...
        try {
            switch (Context.Letter.Body) {
                ...
                case JoinChatRoomCommand joinChatRoom:
                    var joinChatRoomResult = await _grain.JoinChatRoomAsync(joinChatRoom.ChatRoomId, joinChatRoom.UserId, Context.RequestAborted);
                ...
                case RefreshChatRoomCommand refreshChatRoom:
                    await _grain.RefreshChatRoomAsync(refreshChatRoom.ChatRoomId, Context.RequestAborted);
                    Context.Letter.Sender.Tell(SuccessReply.Instance);
                    break;
                case TriggerChatRoomCommand triggerChatRoom:
                    await _grain.TriggerChatRoomAsync(triggerChatRoom.ChatRoomId);
                    Context.Letter.Sender.Tell(SuccessReply.Instance);
                    break;
            }
        } catch (Exception ex) {
